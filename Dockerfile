...
# Installa dipendenze di sistema e Chrome
...
# Debug: stampa la versione di Chrome
RUN google-chrome-stable --version

# Estrai la versione di Chrome e installa il ChromeDriver corrispondente
RUN export CHROME_VERSION=$(google-chrome-stable --version | cut -d " " -f 3 | cut -d "." -f 1) && \
    echo "Installing ChromeDriver for Chrome major version: $CHROME_VERSION" && \
